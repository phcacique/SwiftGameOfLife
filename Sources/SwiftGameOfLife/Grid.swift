...
public class Grid{
    ...
    public init(width:Int = 10, height:Int = 10, isRandom:Bool = false, proportion:Int = 50) {
        self.width = width
        self.height = height
        self.cells = []
        
        if isRandom{
            initRandomGrid(width, height, proportion)
        } else {
    ...
    public func initRandomGrid(_ width: Int, _ height: Int, _ proportion:Int = 50) {
        cells = []
        for i in 0..<width{
            var row:[Cell] = []
            for j in 0..<height{
